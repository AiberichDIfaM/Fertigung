...
class FlexibleJobShopEnv(gym.Env):
    ...
    def _calculate_profit(self):
        ...
        for buf in (
            [self.global_buffer]
            + [m.input_buffer for m in self.machines]
            + [m.output_buffer for m in self.machines]
        ):
            for p in buf:
                val += p.type.value
                cost += p.type.cost
    ...
    def _get_observation(self):
        ...
        for i in range(self.max_buffer):
            if i < len(self.global_buffer):
                idx = next(
                    (j for j, pt in enumerate(self.part_types) if pt.name == self.global_buffer[i].type.name),
                    self.empty_marker,
                )
            else:
        ...
        for m in self.machines:
            ...
            for pt in self.part_types:
                sup = any(
                    any(ip.name == pt.name for ip in tr.input_types) for tr in m.machine_type.transformations
                )
    ...
    def get_action_mask(self):
        ...
        for k in range(1, self.n_actions):
            ...
            if all(avail.get(n, 0) >= c for n, c in req.items()):
                mask[k] = 1
        return mask
    ...
    def step(self, action):
        ...
        for m in self.machines:
            if len(m.current_jobs) < m.machine_type.slots:
                for t in m.transformation_priority:
                    if m.can_start_transformation(t):
                        self.part_id_counter = m.start_transformation(t, self.part_id_counter)
                        break
            ...
            while m.output_buffer:
                ...
                if not self.final_mapping.get(p.type.name, False):
                    self.global_buffer.append(p)
        # refill end
        self.anlage.global_buffer = self.global_buffer
        self.anlage.refill_global_buffer(self.max_buffer)
        self.global_buffer = self.anlage.global_buffer
